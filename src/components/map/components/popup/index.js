...
class Container extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const { latlng, layers, interactionLayers, data, layerSelected } = nextProps;
    if (!isEqual(latlng, this.props.latlng)) {
      this.setPopup(nextProps);
    }
    if (!isEqual(layers, this.props.layers)) {
      this.removePopup();
    } else if (!isEqual(interactionLayers, this.props.interactionLayers) || !isEqual(data, this.props.data) || !isEqual(layerSelected, this.props.layerSelected)) {
